...
async def generate(request: GenerateRequest):
    result = await generate_website(
        chat_history=request.chat_history, data=request.data
    )
    return {
        "status": "success",
        "html": result["html"],
...
async def edit(request: EditRequest):
    ...
    result = await edit_website(
        html=request.html, css=request.css, prompt=request.prompt
    )
    return {"status": "success", "html": result["html"], "css": result["css"]}


@app.post("/api/extract")
async def extract(
    file: Optional[UploadFile] = File(None), link: Optional[str] = Form(None)
):
